  ...
  (dynamic-wind
    ...
    (lambda ()
      (test-case "real llama session follows a finite exact hard language"
        (define result
          (generate backend "Reply with exactly yes or no:"
                    (choice (list (lit " yes") (lit " no")))
      ...
      (test-case "real llama session runs terminal-only PWSG rules"
        (define spec
          (control (text 12)
                   ...
                   (avoid (lit "sorry"))
                   (avoid (lit "unknown"))))
        (define clear-answer (observe backend spec " clear answer"))
        (define clear-only (observe backend spec " clear"))
        (define answer-only (observe backend spec " answer"))
        (define bad-both (observe backend spec " sorry unknown"))
        (define sorry-only (observe backend spec " sorry"))
        (define unknown-only (observe backend spec " unknown"))
        (define calibration
        ...
        (define result
          (generate backend "Give a short direct answer to: What is 2+2?"
                    spec
