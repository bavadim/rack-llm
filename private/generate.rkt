...
(provide compile-spec accepts? observe-token-ids generation-request generate-batch
         ...
         generation-result-attempts generation-result-proposed-tokens
         generation-result-model-draws generation-result-trie-nodes)

(struct context (vocabulary) #:transparent)
(struct compiled (model program) #:transparent)
...
(define (accepts? spec value)
  ...
  (define tok (model-tokenizer (compiled-model spec)))
  (define ids (tokenize tok value))
...
(define (observe-token-ids spec ids)
  ...
  (define st (run spec ids))
  (unless (program-accepting? st) (error 'observe-token-ids "candidate is not accepted by the hard program"))
  (program-observe (compiled-program spec) st ids
                   (lambda (part) (detokenize (model-tokenizer (compiled-model spec)) part))))

(struct request (compiled prompt attempts draws weak temperature max-tokens seed deadline) #:transparent)
(define (generation-request compiled prompt #:max-attempts attempts #:max-model-draws [draws #f]
                            #:weak-model [weak #f] #:temperature [temperature 0.7]
                            #:max-tokens [max-tokens 128] #:seed [seed #f]
                            #:deadline-ms [deadline #f])
  ...
  (request compiled prompt attempts draws weak temperature max-tokens seed deadline))

(struct generation-result
  (status reason token-ids text lm-logprob latency-ms tokenizer-fingerprint
   posterior attempts proposed-tokens model-draws trie-nodes)
  #:transparent)
(struct job (compiled prompt-ids attempts draws weak temperature max-tokens rng trie cache started deadline)
  #:transparent)
(struct proposal (outcome ids logprob proposed draws updates terminal mass) #:transparent)
...
(define (request->job expected r)
  ...
  (define n (program-rule-count (compiled-program spec)))
  (cond [(and (positive? n) (not (request-weak r))) (error 'generate-batch "weak-model-required")]
        [(and (zero? n) (request-weak r)) (error 'generate-batch "weak model requires weak rules")]
        [(and (request-weak r) (not (= n (weak-model-arity (request-weak r)))))
         (error 'generate-batch "weak-model arity does not match program")])
  ...
  (job spec ids (request-attempts r) (request-draws r) (request-weak r)
       (request-temperature r) (request-max-tokens r) (make-rng (request-seed r))
       (make-cars-trie) (make-hasheq) (box #f) (request-deadline r)))

(define (job-model j) (compiled-model (job-compiled j)))

(define (terminal-posterior j ids st node)
  (cond [(not (job-weak j)) 1.0]
        [(and node (hash-ref (job-cache j) node #f))]
        [else
         (define labels
           (program-observe (compiled-program (job-compiled j)) st ids
                            (lambda (part) (detokenize (model-tokenizer (job-model j)) part))))
         (define p (weak-posterior/row (job-weak j) labels))
         (when node (hash-set! (job-cache j) node p)) p]))
(define (terminal j ids st logprob proposed draws updates prefix node)
  (define mass (terminal-posterior j ids st prefix))
  (define old (if node (cars-node-mass node) 1.0))
  (cond [(or (<= old 0.0) (> mass (+ old 1e-12)))
         (proposal 'internal-invalid ids logprob proposed draws updates node mass)]
        [else
         (define accepted?
         ...
         (proposal (if accepted? 'accepted (if (zero? mass) 'threshold-rejected 'posterior-rejected))
                   ids logprob proposed draws updates node mass)]))

(define (attempt j emit remaining reset?)
  (define program (compiled-program (job-compiled j)))
  (define provider (model-provider (job-model j)))
  (define stop (provider-vocab-size provider))
  (define eogs (provider-eog-token-ids provider))
  ...
  (let loop ([depth 0] [rev '()] [st (program-initial program)]
    ...
    (cond
      [(and remaining (>= draws remaining))
       (proposal 'draw-budget (reverse rev) logprob proposed draws '() #f 0.0)]
      [(>= depth (job-max-tokens j))
       (define child (and node (cars-node-child! (job-trie j) node stop 1.0)))
       (if (program-accepting? st)
           (terminal j (reverse rev) st logprob proposed draws updates node child)
           (proposal 'hard-invalid (reverse rev) logprob proposed draws updates child 0.0))]
      [else
       (define domain
       ...
       (cond
         [(member id eogs)
          (if (program-accepting? st)
              (terminal j (reverse rev) st next-log proposed (add1 draws) next-updates node child)
              (proposal 'hard-invalid (reverse rev) next-log proposed (add1 draws)
                        next-updates child 0.0))]
         [else
...
(define (failure j start status reason attempts proposed draws)
  (generation-result status reason '() "" #f (- (current-inexact-milliseconds) start)
                     (tokenizer-fingerprint (model-tokenizer (job-model j))) #f attempts proposed draws
                     (cars-trie-node-count (job-trie j))))
(define (sample j emit)
  (define start (current-inexact-milliseconds)) (set-box! (job-started j) start)
  (let loop ([number 1] [rejected 0] [proposed 0] [draws 0])
    (cond
             ...
             [else
              ...
              (if (eq? (proposal-outcome p) 'accepted)
                  (generation-result 'found #f (proposal-ids p)
                                     ...
                                     (and (job-weak j) (proposal-mass p)) number
                                     next-proposed next-draws (cars-trie-node-count (job-trie j)))
                  (loop (add1 number) (add1 rejected) next-proposed next-draws))])])))
